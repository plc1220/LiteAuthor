...
export const MOTIFS_WIKI_PATH = 'story/motifs.md';
...
export function MotifTrackerPanel({onNavigate}: MotifTrackerPanelProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const file = await api.wikiGet(activeProject.id, MOTIFS_WIKI_PATH);
  ...
  if (!activeProject) {
    return <p className="text-sm text-ink-muted italic">Select a project to load tracked motifs.</p>;
  }
    ...
    <div>
      <section className="mb-5 border-b border-oak-variant pb-5">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {motifs.map((motif) => {
            const isSel = motif.name === selected?.name;
            const total = isSel ? occurrences.reduce((sum, o) => sum + o.count, 0) : 0;
            return (
              <button
                key={motif.name}
                type="button"
                className={`rounded-sm border px-3 py-2 text-sm font-serif ${
                  isSel ? 'border-primary bg-sepia-highest text-primary' : 'border-oak-variant bg-sepia-low text-ink hover:border-primary'
                }`}
                onClick={() => setSelectedName(motif.name)}
              >
                {motif.name}
                {isSel && total > 0 ? <span className="ml-2 font-sans text-[10px]">{total}</span> : null}
              </button>
            );
          })}
          {motifs.length === 0 ? <span className="text-sm text-ink-muted italic">Add ## headings in story/motifs.md to define tracked motifs.</span> : null}
        </div>
      </section>

      <div className={`grid gap-5 ${motifs.length > 0 ? 'lg:grid-cols-[minmax(0,1fr)_300px]' : 'max-w-[860px]'}`}>
            ...
            <div className="flex flex-wrap gap-2">
              {heat.map((cell) => (
                <div key={cell.id} className="group relative">
                  ...
                  <div className="absolute z-20 hidden min-w-44 border border-oak-variant bg-parchment-bright p-2 text-xs shadow-xl group-hover:block top-12 left-0">
            ...
            <div className="space-y-3">
              ...
              {occurrences.map((occurrence) => (
                <button
                  ...
                  type="button"
                  className="w-full rounded-sm border border-oak-variant bg-parchment-bright/40 p-4 text-left hover:border-primary"
                  onClick={() => {
                    setActiveScene(occurrence.sceneId);
                    onNavigate('ZenEditor', 'push');
                  }}
                >
                  <div className="flex justify-between gap-4 font-sans text-xs uppercase tracking-widest text-ink-muted">
                    <span>
                      {occurrence.chapterTitle} · {occurrence.sceneTitle}
                    </span>
        ...
        {motifs.length > 0 ? (
          <aside className="h-fit rounded-sm border border-oak-variant bg-sepia-low p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <BookOpen className="h-4 w-4 text-oak" />
            </div>
            <div className="max-h-[min(420px,50vh)] overflow-y-auto rounded-sm border border-oak-variant bg-parchment-bright p-4">
