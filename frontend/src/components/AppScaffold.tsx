...
const NAV_ITEMS: {label: string; screen: Screen; active: AppSection; icon: ReactNode; transition: TransitionType}[] = [
  ...
  {label: 'Project Desk', screen: 'StoryWikiHub', active: 'desk', icon: <Library className="h-4 w-4" />, transition: 'push'},
  {label: 'Settings', screen: 'SettingsScreen', active: 'settings', icon: <Settings className="h-4 w-4" />, transition: 'push'},
];

export function AppScaffold({active, children, actions, footer, mainClassName = 'overflow-y-auto', onNavigate}: AppScaffoldProps) {
  const activeProject = useProjectStore((s) => s.activeProject);

  return (
    <div className="flex h-screen min-h-screen flex-col bg-parchment text-ink paper-grain">
      <header className="relative z-50 flex min-h-12 flex-wrap items-center justify-between gap-3 border-b border-oak-variant bg-sepia-low/95 px-4 py-2 md:px-7">
          ...
          <nav className="flex min-w-0 items-center gap-1 text-xs font-sans uppercase tracking-widest">
            ...
            {NAV_ITEMS.map((item) => (
              <button
                ...
                onClick={() => onNavigate(item.screen, item.transition)}
              >
                {item.icon}
        ...
        <div className="flex min-w-0 items-center gap-2">
          {activeProject ? (
            <span className="hidden max-w-[220px] truncate font-serif text-sm italic text-ink-muted lg:inline" title={activeProject.name}>
              {activeProject.name}
            </span>
          ) : null}
          {actions}
